...
d3.easygraph.presets = {
  ...
  pressureInhg:     { label: 'Pressure',         unit: 'inHg',       scale: 'linear',    convert: _withRounding(function(v) { return v * 0.02953; }),   range: [ 28, 31 ] },
  ...
  temperatureF:     { label: 'Temperature',      unit: '\xB0F',      scale: 'linear',    convert: _withRounding(function(v) { return v * 1.8 + 32; }),  range: [ -10, 110 ] },
  windSpeed:        { label: 'Wind Speed',       unit: 'm/s',        scale: 'linear',    convert: _roundedIdentity,                              range: [ 0, 20 ] },
  windSpeedKmph:    { label: 'Wind Speed',       unit: 'kmph',       scale: 'linear',    convert: _withRounding(function(v) { return v * 3.6; }),       range: [ 0, 80 ] },
  windSpeedMph:     { label: 'Wind Speed',       unit: 'mph',        scale: 'linear',    convert: _withRounding(function(v) { return v * 2.23694; }),   range: [ 0, 50 ] },
  ...
  rainFallInches:   { label: 'Rain Fall',        unit: 'inches',     scale: 'linear',    convert: _withRounding(function(v) { return v / 25.4; }),      range: [ 0, 100 ] },
