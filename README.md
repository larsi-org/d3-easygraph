...
- **Scatter** — [Weather](https://larsi.org/weather/)'s "Pressure & Wind" tab (`voronoi: true`,
  ...
  [Air Traffic](https://larsi.org/air-traffic/) (`color.domain` for a fixed altitude scale,
  `labels`/`labelMinZoom` for callsigns once zoomed in) — both overlaid on a D3-drawn US map via a
  shared pan/zoom helper (not part of this repo) that drives `graph.rescale(k)` to keep points and
  arrows a constant size through the zoom
